...
import { getMessages, sendMessage } from "../features/site/siteSlice";

const ChatDialog = (props: any) => {
    const [message, setmessage] = useState();
    const dispatch = useAppDispatch();
    const messages = useAppSelector((state) => state.site.messages);
    ...
    const handleSendMessage = () => {
        ...
        // @ts-ignore
        setmessage("");
        setTimeout(() => dispatch(getMessages(threadId)), 3000);
    };

    return (
        <div className={css`overflow-y: auto; position: fixed; z-index: 1000; bottom: 0px; right: 0px; width: 37%; height: 55%; background-color: rgb(241, 241, 241);`}>
            ...
            <MDBRow className={css`padding-left: 10px; padding-right: 10px; margin-top: -7.352px;`}>
                ...
                <p className={css`font-family: 'Lexend Deca', sans-serif; border-radius: 15px 30px 30px 5px; padding: 2.55vh; min-height: 10vh; width: 70%; background-color: ${COLORS.WHITE_1};`}>
                    ...
                    />
                    <IoIosSend onClick={handleSendMessage} className={css`color: white; border: 1px solid #673ED1; padding: 1.77vh; background-color: #673ED1; width: 7vh; height: 7vh; border-radius: 50%; position: fixed; right: 0.88vh; bottom: 1.10vh;`} color="white" />
                </div>
            </MDBRow>
        </div>
    );
